...
void setup() {
  ...
  mySerial.println(F("I-S"));        //Mensajes para determinar correcto funcionamiento en el PC
  ...
  while (!modem.waitForNetwork()) {                          //Esperamos en un bucle infinito la conexion de red.
    mySerial.println(F("E-R"));
  }
  if (modem.isNetworkConnected()) {
    mySerial.println(F("R-E"));      //Mostrara un mensaje cuando de haya conectado a la red.
  }
...
void loop() {
  switch (ESTADO) {
    ...
    case 4:
      ...
      break;
    case 5:
     // INDICADOR_DE_ENVIO = modem.callHangup();               //Este comando esta disponible en la libreria tinyGSM y sirve para terminar una llamada,
      /* mySerial.println(F("LLAMADA TERMINADA"));              //Desplegamos información en la terminal para saber en que momento termina la llamda.
        mySerial.println(INDICADOR_DE_ENVIO);                  //Mostramos el resultado de la llamada (Normalmente si se tarda es porque ocurrió algún error :'v ).
        mySerial.println(F("COLOCANDO EL SIM800 EN SLEEP"));
        mySerial.println(F(""));
    ...
    case 6:
      ...
      break;
    case 7:
      //mySerial.println(F("ACTIVANDO INTTERUPCIONES"));
      //  mySerial.println(F("MAQUINA DE ESTADOS FINITOS FINALIZADA"));
    ...
    case TEXT:
      ...
      if (SMS_ESTATE == 0) {
        /*
          HUMITY = 0;
          TEMP = 0;
          while ((HUMITY < 1) || (TEMP < 1)) {*/
        DTH();
        /*  SECURE_COUNTER++;
          delay(111);
          if (SECURE_COUNTER > 11) {
            HUMITY = 1;
            TEMP = 1;
          }
          }*/
    ...
    case ALARMS:
      ...
      sendData(CONSTRUCTOR_AT, 1111, 1);                                      //Enviamos nuestro constructor AT con nuestra alarma ya programada.
