...
const String Comando3 = ":00:00\",3,0";
int SETED;



String CONSTRUCTOR_AT;                                //String en la cual se generara el comando AT para programar la siguiente alarma.
String CONSTRUCTOR_AT2;                               //String en la cual se generara el comando AT para programar la alarma de backup
...
void setup() {
  ...
  mySerial.println(F("VERSION_SSM5_ALPHA_ASTABLE.2"));
...
mySerial.flush();
    ...
    case LLAMADA:
      ...
      { //Esperamos en un bucle infinito la conexion de red.
        mySerial.println(F("2"));
      }
    ...
    case 12:
      ...
      if (!SENDER)
      {
        //Serial.println(F("AT+CMGS=\"0017867084206\""));
         sendData("AT+CMGS=\"0017867084206\"", 1111, 1);
      }
      if (SENDER)
      {
    ...
    case ALARMS:
      ...
      HORA_DE_ENVIO = HORA_ACTUAL_INT + 6;                                    //Le sumamos uno para determinar de envio del siguiente mensaje, en este caso una hora despues. (Falta construir el comando para programarlo)
       HORA_DE_ENVIO2 = HORA_ACTUAL_INT + 7;   
        ...
        if (HORA_DE_ENVIO < 10) 
        {                                               //Empezamos a construir nuestro comando AT para programarle la hora del siguiente envio SMS
        ...
        if (HORA_DE_ENVIO >= 10) 
        {
         ...
         if (HORA_DE_ENVIO2 > 23) 
        {
          HORA_DE_ENVIO2 =  HORA_DE_ENVIO2 - 24;
        }
        if (HORA_DE_ENVIO2 < 10) 
        {                                               //Empezamos a construir nuestro comando AT para programarle la hora del siguiente envio SMS
          CONSTRUCTOR_AT2 += Comando1_1;
          CONSTRUCTOR_AT2 += String(HORA_DE_ENVIO2);
          CONSTRUCTOR_AT2 += Comando3;
        }
        if (HORA_DE_ENVIO2 >= 10) 
        {
          CONSTRUCTOR_AT2 += Comando1_2;
          CONSTRUCTOR_AT2 += String(HORA_DE_ENVIO2);
          CONSTRUCTOR_AT2 += Comando3;
        }
        ...
        mySerial.println(CONSTRUCTOR_AT2);
        mySerial.println(F(""));
        ...
        sendData("AT+CALD=3", 1111, 1);  
        sendData(CONSTRUCTOR_AT2, 1111, 1);
        SETED = ALARM_SETED.indexOf("ERROR");
        if (SETED != -1)
        {
          RETRY_SETED();
        }
