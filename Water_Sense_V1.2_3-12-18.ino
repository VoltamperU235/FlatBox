...
void LOW_SIGNAL()
{
  ...
  StaticJsonBuffer<85> jsonBuffer;                           //Generar un buffer para la String Json que contrendra los datos.
...
void SET_ALARM()
{
  ...
  // ESTATE = 0; primer bug encontrado;
}

void JSON()
{
  StaticJsonBuffer<115> jsonBuffer;                           //Generar un buffer para la String Json que contrendra los datos.
  JsonObject& root = jsonBuffer.createObject();               //Se crea un Objeto del tipo JsomObjet para ingresar los datos.

  String pos =  modem.getGsmLocation();
  int batt = modem.getBattPercent();
  String imeiid = modem.getIMEI();

  root["P"] = pos;                  //Obtenemos la localización (Se nesecita conexion GPRS :'v).
  root["L"] = LOCATION;
  root["B"] = batt;
  root["I"] = imeiid;
